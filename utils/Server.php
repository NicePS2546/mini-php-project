...
class Server
{
    ...
    public function register($conn,$table_user,$table_userInfo,$username, $email, $passwordHash)
    {
      try {
        ...
        $user_status = $this->register_user( $conn, $table_user,$username,  $passwordHash, $email );
        $user_id = $conn->lastInsertId();

        $sql = "INSERT INTO $table_userInfo (id, email) VALUES (:id, :email)";
        $stmt = $conn->prepare($sql);
        $user_info_status = $stmt->execute(["id"=> $user_id, "email"=> $email ]);
         

        $conn->commit();
        return $user_status && $user_info_status;
      } catch (Exception $e) {
        $conn->rollBack();
        return $e;
